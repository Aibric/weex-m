...
export default {
    initFont (fontFamily = 'iconfont', src = getUrl('font_404010_f29c7wlkludz33di.ttf')) {
    ...
    css(obj, style, value) {
        ...
        if (new Object(style) === style) {
            ...
            if (value === undefined) {
                return this.getStyle(obj, style)
            }
    ...
    getStyle(obj, attr){  
        if (obj.style && obj.style[attr]) {
            return obj.style[attr]
        } else if (obj.$el && obj.$el.style && obj.$el.style[attr]) {
            return obj.$el.style[attr]
        } else if (obj.$el && obj.$el[attr]) {
            return obj.$el[attr]
        } else if (obj[attr]) {
            return obj[attr]
    ...
    toUpperCaseAll(str, stat = false, isKey = false) {
        let index = 0
        if (isObject(str)) {
            let newObj = {}
            for (let key in str) {
                newObj[this.toUpperCaseAll(key, stat)] = isKey 
                    ? str[key] 
                    : this.toUpperCaseAll(str[key], stat)
            }
            return newObj;
        }
