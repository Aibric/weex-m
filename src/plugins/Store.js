...
function Es5Proxy(target, handle) {
    ...
    Object.keys(target2).forEach(function(key){
        Object.defineProperty(target, key, {
            get: function() {
                return handle.get(target2, key);
            },
            set: function(newVal) {
                handle.set && handle.set(target2, key, newVal);
            },
            // writable: true,
            enumerable : true,
        });
...
function mapGetters (getters, callback, privateData) {
    ...
    for (let key in getters) {
        ...
        let val2 = false
        if (val.length >= 2) {
            val2 = val[1].trim().toLocaleLowerCase() == 'as' 
                    ? val[2].trim()
                    : val[1].trim()
            val = val[0].trim()
        } else {
        ...
        if (typeof getters[key] !== 'string' ||
            !isFunction(privateData.get(this).getters[val])) continue
        data[val2 ? val2 : val] = callback === privateData.get(this)._tag
            ? privateData.get(this).getters[val](this.state)
            : () => privateData.get(this).getters[val](this.state) 
    }
...
function dispatchRoot (dispatch, data, privateData) {
    ...
    return privateData.get(this).actions[dispatch]({
        ...
        mapGetters: (...args) => this.mapGetters.call(this, ...args, privateData.get(this)._tag),
        state: this.states(),
    }, data)
}
...
const Store = (function() {
    ...
    function Store(storeData) {
        ...
        this.mapGetters = function (getters, callback) {
            if (privateData.get(this)._tag !== callback || this.isRun && !privateData.get(this).set) {
